...
select count(mabm) from giaovien
where mabm = 'httt';

-- Câu 3: Tính số lượng giáo viên có người quản lý về mặt chuyên môn
select count(gvqlcm) from giaovien
...
select giaovien.MAGV, giaovien.HOTEN, count(madt) from giaovien 
inner join thamgiadt on giaovien.MAGV = thamgiadt.MAGV group by MAGV;
...
select bomon.TENBM, count(magv) from giaovien 
join bomon on bomon.MABM = giaovien.MABM group by TENBM;
-- ***Dùng GROUP BY + HAVING
-- Câu 11: Cho biết những bộ môn từ 2 giáo viên trở lên.
select tenbm from giaovien join bomon on giaovien.MABM = bomon.mabm 
group by tenbm having count(MAGV) >= 2;
-- Câu 12: Cho tên những giáo viên và số lượng đề tài của những GV tham gia từ 3 đề tài trở
-- lên.
select giaovien.HOTEN, count(thamgiadt.MADT) from giaovien 
join thamgiadt on giaovien.MAGV = thamgiadt.MAGV group by hoten having count(madt)> 2;
-- Câu 13: Cho biết số lượng đề tài được thực hiện trong từng năm.
select year(ngaybd), count(MADT) from detai group by ngaybd order by ngaybd  desc ;
-- ************************************************
...
select magv,hoten,luong from giaovien where luong >
(select luong from giaovien where magv = 001);
-- Câu A4: Cho biết họ tên những giáo viên mà không có một người thân nào
select giaovien.HOTEN from giaovien join bomon on giaovien.magv = nguoithan.MAGV group by hoten;
